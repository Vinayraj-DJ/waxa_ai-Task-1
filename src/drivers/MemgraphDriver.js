...
export class MemgraphDriver extends BaseDriver {
  constructor(config, platformSpecs = {}) {
    super('Memgraph Cloud', platformSpecs);
  ...
  async connect() {
    try {
      ...
      console.log(`[Memgraph] Connected to ${this.connectionConfig.uri}`);
    } catch (error) {
      console.warn(`[Memgraph] Connection failed: ${error.message}`);
  ...
  async createIndexes() {
    ...
    try {
      ...
      await session.run('CREATE INDEX ON :User(city, age)');
    } catch (e) {
      // Memgraph index syntax tolerance
    } finally {
      await session.close();
    }
  }
  ...
  async getObservableFootprint() {
    return {
      storedDataSizeMb: "18.4 MB",
      memoryUsageMb: "110 MB (In-Memory SkipList)",
